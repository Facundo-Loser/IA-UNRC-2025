...
CANT_REINAS = 4    # N >= 1 o N >= 4
POPULATION_SIZE = 20

# Fitness function (chequea si hay reinas en la misma fila o columnas)
    ...
    for i in range(len(individual)):
        for j in range(i + 1, len(individual)):
            if individual[i] == individual[j] or abs(individual[i] - individual[j]) == abs(i - j):
                reinas_atacadas += 1
    return reinas_atacadas

# crea una posicion (x,y) de la reina
def create_individual():
    return [random.randint(0,CANT_REINAS-1) for _ in range(CANT_REINAS)]
...
def crossover(p1, p2):
    ...
    if random.random() < CXPB: # random.random() returns a floating point number in [0.0, 1.0)
        point = random.randint(1, CANT_REINAS-1)
...
def mutate(individual):
    if random.random() < MUTPB:
        pos = random.randint(0, CANT_REINAS-1)
        individual[pos] = random.randint(0, CANT_REINAS-1)
    return individual
